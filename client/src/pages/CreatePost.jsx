...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://dalle-clone-dvzp.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch('https://dalle-clone-dvzp.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
    ...
    <section className="max-w-7xl mx-auto">
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        ...
        <div className="relative bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-64 p-3 h-64 flex justify-center items-center">
          {form. photo ? (
            <img src={form.photo} alt={form.prompt} className="w-fuil h-full object-contain" />
          ) : (
        ...
        <div className="mt-10">
          <p class="mt-2 text-[#666e75] text-[14px]">Once you have created the image you want, you can share it with the others in the community</p>
